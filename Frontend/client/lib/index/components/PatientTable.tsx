...
export function PatientTable({ rows, compact = false }: PatientTableProps) {
  return (
    <div className="overflow-x-auto">
      <table className="w-full min-w-[720px] text-left">
        <thead>
          <tr className="text-[10px] font-bold uppercase tracking-[.1em] text-[#9aaeb4]">
            <th className="px-5 py-3">HBCR ID</th>
        ...
        <tbody>
          {rows.map((p, i) => (
            <tr
              key={p.id}
              className={`border-t border-[#edf3f4] text-xs ${!compact && i === 0 ? "" : ""}`}
            >
              <td className="px-5 py-4 font-bold text-[#087888]">{p.id}</td>
              ...
              <td className="px-3 py-4 font-bold text-[#52707b]">{p.stage}</td>
              <td className="px-3 py-4">
                <span
              ...
              <td className="px-5 py-4 text-[#80959c]">{p.date}</td>
            </tr>
          ))}
        </tbody>
      </table>
