...
export function extractPatient(v: Values): {
  ...
  const gender = (val(v, "11. Gender") ?? "Male").toLowerCase().startsWith("f")
    ? "FEMALE"
    : (val(v, "11. Gender") ?? "Male").toLowerCase().startsWith("m")
      ? "MALE"
      : "OTHER";
  ...
  return {
    ...
    middleName,
    lastName,
    age: val(v, "10. Age") || null,
    dateOfBirth: val(v, "9. Date of Birth"),
    gender,
...
export function extractAddresses(v: Values): Array<{
  ...
  const res = {
    ...
    durationStay: (() => {
      const raw = val(v, "14(b).Duration of Stay at the above address (in years)");
...
export function extractHabits(v: Values): Array<{
  ...
  return items
    .map((item) => {
      const answer = String(v[`17(a). Habits::${item}::answer`] ?? "No") as "Yes" | "No";
      return {
...
export function extractComorbidities(v: Values): Array<{
  ...
  return items
    .filter((c) => String(v[`17(b). Co-Morbidities::${c}::answer`] ?? "No") === "Yes")
    .map((c) => ({
...
export function extractRegistration(
  ...
  const referralType =
    REFERRAL_TYPE_MAP[val(v, "6. Type of referral") ?? "Self"] ?? "UNKNOWN";
  const caseThrough = val(v, "5. Case Registered Through (Patient’s first reporting at RI)");
  const marital = val(v, "15. Marital status") ?? "";
  const education = val(v, "16. Education") ?? "";
  ...
  const referenceNo = val(v, "1(b).Reference Number") ?? undefined;
  const hbcrRegistrationNo = val(v, "2.Registration Number") ?? undefined;
  ...
  return {
    ...
    dateOfReporting: val(v, "4. Date of reporting") ?? undefined,
    ...
    referralType,
    referralFacilityName: val(v, "6(a). Name of Facility.") ?? undefined,
    referralFacilityHospitalLabNh: val(v, "6(b). Hospital / LAB / N.H.") ?? undefined,
    referralFacilityCity: val(v, "6(c). City") ?? undefined,
    referralFacilityDistrict: val(v, "6(d). District") ?? undefined,
    referralFacilityPincode: val(v, "6(e). Pincode") ?? undefined,
    referralFacilityRegDate: val(v, "6(f). Date of Registration") ?? undefined,
    dateOfFirstDiagnosis: val(v, "7. Date of first diagnosis") ?? undefined,
    ...
    maritalStatusOther: val(v, "15(a). Marital status (Other)") ?? undefined,
    education: education ? EDU_ENUM_MAP[education] ?? education.replace(/[^A-Za-z0-9]+/g, "_").replace(/_+$/, "").toUpperCase() : undefined,
    educationOther: val(v, "16(a). Education (Other)") ?? undefined,
    occupation: val(v, "Occupation") ?? undefined,
    remarks: val(v, "Remarks") ?? undefined,
    contactNumber: val(v, "32. Contact Number") ?? undefined,
    designation: val(v, "33. Designation") ?? undefined,
    formCompletedBy: val(v, "30. Name of person completing form (IN CAPITALS)") ?? undefined,
    formCompletionDate: val(v, "31. Date of completion of form") ?? undefined,
  };
}
...
export function extractPathology(v: Values): Partial<ApiPathologicalDiagnosis> {
  ...
  const longest = num("20. Longest duration of symptom for cancer (in months)");
  if (longest !== undefined) map.longestSymptomDurationMonths = longest;

  const text = (k: string) => val(v, k) ?? undefined;
  map.anatomicalSite = text("21(a). Anatomical Site of Specimen / Biopsy / SMEAR");
  map.pathologySlideNo = text("21(b). Pathology Slide No");
  map.primaryTumorSite = text("21(d). Primary Site of Tumour - Topography");
  map.morphology = text("21(e). Primary Histology / Morphology");
  ...
  map.pathologyDateOfReporting = text("21(c). Date of Reporting");
